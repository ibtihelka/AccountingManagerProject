...
public class NumberRcsFinder extends KeyFinder<String> implements Serializable {
    ...
    public NumberRcsFinder() throws Exception {

        super(new Key(KeyType.NUMERO_RCS));
    }
    ...
    private boolean _CheckValue(KeyValue keyValue, _Out<String> value) {
        ...
        for (int i = 0; i < rcsStr.length(); i++) {
            if (i + 9 < rcsStr.length())
                value.set(StringHelper.substring(rcsStr, i, 9));
            else
                value.set(StringHelper.substring(rcsStr, i, rcsStr.length() - i));

            if (NumberHelper.IsRcs(value.get()))
                return true;
        }
