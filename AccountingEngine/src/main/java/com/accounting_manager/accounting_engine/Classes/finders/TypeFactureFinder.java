...
public class TypeFactureFinder extends KeyFinder<Boolean> implements Serializable {
    ...
    public TypeFactureFinder() throws Exception {
        super(new Key(KeyType.TYPE_FACTURE));
    }
    ...
    public List<TextBlock> FindValue(KeyValue keyBlock, List<Block> allBlocks, _Out<Boolean> isValueChecked, _Out<Boolean> checkResult) {
        List<TextBlock> res = new ArrayList<>();
        res.add(keyBlock.getKeyBlocks().get(0));
        return res;
    }
