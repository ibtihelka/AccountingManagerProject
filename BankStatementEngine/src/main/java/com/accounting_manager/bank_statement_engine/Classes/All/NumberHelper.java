package com.accounting_manager.bank_statement_engine.Classes.All;
...
public class NumberHelper implements Serializable {
    ...
    public static boolean IsAccountNumber(String accountNumber) {
        ...
        if (accountNumber.length() == 11) {
            for (int e = 0; e < accountNumber.length(); e++) {
                // Tester sur l'existantce des caractère non digital.
                if (!NumberHelper.IsDigit(accountNumber.charAt(e)))
                    return false;
            }
            return true;
        } else
    ...
    public static boolean IsBicNumber(String bicNumber) {
        int alphabet = 0;
        int numeric = 0;

        // Tester sur la longeur de la chaine.
        if (bicNumber.length() == 11 || bicNumber.length() == 8) {
            for (int e = 0; e < bicNumber.length(); e++) {
                // Convert the character to its ASCII code
                int asciiCode = (int) bicNumber.charAt(e);
                if (asciiCode >= 65 && asciiCode <= 90) { // check if it's an uppercase alphabetic character
                    alphabet++;
                }
                else if (asciiCode >= 48 && asciiCode <= 57) { // check if it's a numeric character
                    numeric++;
                }
                else
                    return false;
            ...
            return numeric!= bicNumber.length() && alphabet>=numeric;
        } else
            return false;

    }

    public static boolean IsIbanNumber(String ibanNumber) {
        // Tester sur la longeur de la chaine.
        if (ibanNumber.length() == 27 && ibanNumber.startsWith("FR")) {
            for (int e = 2; e < ibanNumber.length(); e++) {
                // Tester sur l'existantce des caractère non digital.
                if (!NumberHelper.IsDigit(ibanNumber.charAt(e)))
                    return false;
            }
            return true;
        } else
    ...
    public static boolean IsRibNumber(String ribNumber) {
        // Tester sur la longeur de la chaine.
        if (ribNumber.length() == 25) {
            for (int e = 0; e < ribNumber.length(); e++) {
                // Tester sur l'existantce des caractère non digital.
                if (!NumberHelper.IsDigit(ribNumber.charAt(e)))
                    return false;
            }
            return true;
        } else
    ...
    public static boolean IsTva(_Out<String> tva) {
        ...
        if (tva.get().length() == 13) {
            ...
            countryIdentifier = StringHelper.substring(tva.get(), 0, 2);
            // Verification.
            return NumberHelper.IsAccountNumber(rcs) && countryIdentifier.equals("FR");
        } else
            return false;
    }
...
//			}
    ...
    /// <param name="tva">Numéro de tva.</param>
    /// <returns>Numero rcs.</returns>
    public static String GetRibFromIban(String iban) {
