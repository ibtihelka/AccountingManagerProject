...
@Log4j2
public class BicNumberFinder extends KeyFinder<String> implements Serializable {
    ...
    public BicNumberFinder() throws Exception {

        super(new Key(KeyType.BIC_NUMBER));
    }

    public BicNumberFinder(LearningEngine learningEngine) throws Exception {

        super(new Key(KeyType.BIC_NUMBER), learningEngine, null);
    }
    /// <summary>
    ...
    private boolean _CheckValue(KeyValue keyValue, _Out<String> value) {
        String bicNumberStr = TextBlock.GetSequenceString(keyValue.getValueBlocks());

        for (int i = 0; i < bicNumberStr.length(); i++) {
            if (i + 11 < bicNumberStr.length())
                value.set(StringHelper.substring(bicNumberStr, i, 11));
            else
                value.set(StringHelper.substring(bicNumberStr, i, bicNumberStr.length() - i));

            if (NumberHelper.IsBicNumber(value.get()))
                return true;
        }
    ...
    public LearningEntry GetLearningEntry(KeyValue keyValue, _Out<Boolean> isValidValue) {
        if (isValidValue.get()) {
            String bicNumber = GetValue(keyValue);
            return this.getLearningEngine().GetEntry(KeyType.BIC_NUMBER, bicNumber);
        } else
            return null;
    }
