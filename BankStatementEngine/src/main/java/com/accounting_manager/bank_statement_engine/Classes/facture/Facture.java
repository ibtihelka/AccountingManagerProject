...
import com.accounting_manager.bank_statement_engine.Classes.finders.*;
...
import com.accounting_manager.bank_statement_engine.Classes.document.DocumentProcessing;
...
public class Facture implements Serializable {
    ...
    private static void mergeFactureInfos(FactureInfo basicInfos, FactureInfo newInfos) {
        if (basicInfos != null) {
            ...
            if (newInfos.isHasAccountNumber()) {
                basicInfos.setAccountNumberKeyValue(newInfos.getAccountNumberKeyValue());
                basicInfos.setAccountNumberKeyValues(KeyValue.Merge(basicInfos.getAccountNumberKeyValues(), newInfos.getAccountNumberKeyValues()));
            }

            if (newInfos.isHasBicNumber()) {
                basicInfos.setBicNumberKeyValue(newInfos.getBicNumberKeyValue());
                basicInfos.setBicNumberKeyValues(KeyValue.Merge(basicInfos.getBicNumberKeyValues(), newInfos.getBicNumberKeyValues()));
            }

            if (newInfos.isHasIbanNumber()) {
                basicInfos.setIbanNumberKeyValue(newInfos.getIbanNumberKeyValue());
                basicInfos.setIbanNumberKeyValues(KeyValue.Merge(basicInfos.getIbanNumberKeyValues(), newInfos.getIbanNumberKeyValues()));
            }

            if (newInfos.isHasRibNumber()) {
                basicInfos.setRibNumberKeyValue(newInfos.getRibNumberKeyValue());
                basicInfos.setRibNumberKeyValues(KeyValue.Merge(basicInfos.getRibNumberKeyValues(), newInfos.getRibNumberKeyValues()));
            }
            ...
            if (newInfos.isHasPeriodStartDate()) {
                basicInfos.setPeriodStartDateKeyValue(newInfos.getPeriodStartDateKeyValue());
                basicInfos.setPeriodStartDateUsedKeyValues(KeyValue.Merge(basicInfos.getPeriodStartDateUsedKeyValues(), newInfos.getPeriodStartDateUsedKeyValues()));
            }

            if (newInfos.isHasPeriodEndDate()) {
                basicInfos.setPeriodEndDateKeyValue(newInfos.getPeriodEndDateKeyValue());
                basicInfos.setPeriodEndDateUsedKeyValues(KeyValue.Merge(basicInfos.getPeriodEndDateUsedKeyValues(), newInfos.getPeriodEndDateUsedKeyValues()));
            }
    ...
    private static boolean extractHtTvaTtc(
            HtFinder htFinder, TvaFinder tvaFinder, TtcFinder ttcFinder, TvaSuppFinder tvaSuppFinder, BicNumberFinder bicNumberFinder,
            boolean isSecondProcessing,
            List<_Document> documents, FactureInfo factureInfo) {
        if (bicNumberFinder == null) {
            ...
            bicNumberFinder.Find(documents);
        ...
        List<KeyValue> tvaSuppKeyValues = bicNumberFinder == null ? tvaSuppFinder.GetAllKeyValues() : null;
        List<KeyValue> amountKeyValues = bicNumberFinder != null ? bicNumberFinder.GetAllKeyValues() : null;
...
//		allDocuments.set(Arrays.copyOfRange(includeDocuments, 0, includeDocuments.length));
    ...
    private FactureInfo _StartProcessing(List<_Document> documents, FactureInfo factureInfo) throws Exception {
        ...
        try {
            if (factureInfo == null || !factureInfo.isHasNumeroSiret()) {
        ...
        // Account Number.
        AccountNumberFinder accountNumberFinder = null;
        try {
            if (factureInfo == null || !factureInfo.isHasAccountNumber()) {
                accountNumberFinder = new AccountNumberFinder(learningEngine);
                accountNumberFinder.Find(documents);
                _factureInfo.setAccountNumberKeyValues(accountNumberFinder.GetAllKeyValues());
                // Dévalider les valeurs non valides.
                if (accountNumberFinder.getValidValue() != null && (factureInfo == null || !factureInfo.isHasAccountNumber())) {
                    accountNumberFinder.getValidValue().setValue(accountNumberFinder.GetValue());
                    _factureInfo.setAccountNumberKeyValue(accountNumberFinder.getValidValue());
                    rcs = _factureInfo.getAccountNumber();
                }
            }
        } catch (Exception e) {
            log.error("Account number not found :" + e);
        }

        // BIC Number.
        BicNumberFinder bicNumberFinder = null;
        try {
            if (factureInfo == null || !factureInfo.isHasBicNumber()) {
                bicNumberFinder = new BicNumberFinder(learningEngine);
                bicNumberFinder.Find(documents);
                _factureInfo.setBicNumberKeyValues(bicNumberFinder.GetAllKeyValues());
                // Dévalider les valeurs non valides.
                if (bicNumberFinder.getValidValue() != null && (factureInfo == null || !factureInfo.isHasBicNumber())) {
                    bicNumberFinder.getValidValue().setValue(bicNumberFinder.GetValue());
                    _factureInfo.setBicNumberKeyValue(bicNumberFinder.getValidValue());
                }
            }
        } catch (Exception e) {
            log.error("BIC number not found :" + e);
        }


        // RIB Number.
        RibNumberFinder ribNumberFinder = null;
        try {
            if (factureInfo == null || !factureInfo.isHasIbanNumber()) {
                ribNumberFinder = new RibNumberFinder(learningEngine);
                ribNumberFinder.Find(documents);
                _factureInfo.setIbanNumberKeyValues(ribNumberFinder.GetAllKeyValues());
                // Dévalider les valeurs non valides.
                if (ribNumberFinder.getValidValue() != null && (factureInfo == null || !factureInfo.isHasIbanNumber())) {
                    ribNumberFinder.getValidValue().setValue(ribNumberFinder.GetValue());
                    _factureInfo.setIbanNumberKeyValue(ribNumberFinder.getValidValue());
                }
            }
        } catch (Exception e) {
            log.error("RIB number not found :" + e);
        }

        // IBAN Number.
        IbanNumberFinder ibanNumberFinder = null;
        try {
            if (factureInfo == null || !factureInfo.isHasIbanNumber()) {
                ibanNumberFinder = new IbanNumberFinder(learningEngine);
                ibanNumberFinder.Find(documents);
                _factureInfo.setIbanNumberKeyValues(ibanNumberFinder.GetAllKeyValues());
                // Dévalider les valeurs non valides.
                if (ibanNumberFinder.getValidValue() != null && (factureInfo == null || !factureInfo.isHasIbanNumber())) {
                    ibanNumberFinder.getValidValue().setValue(ibanNumberFinder.GetValue());
                    _factureInfo.setIbanNumberKeyValue(ibanNumberFinder.getValidValue());
                    _factureInfo.setRibNumber(NumberHelper.GetRibFromIban(_factureInfo.getIbanNumber()));
                }
            }
        } catch (Exception e) {
            log.error("IBAN number not found :" + e);
        }

        // Numéro Tva.
        try {
            if (factureInfo == null || !factureInfo.isHasNumeroTva()) {
                NumberTvaFinder numberTvaFinder = new NumberTvaFinder(numberSiretFinder, accountNumberFinder, learningEngine);
        ...
        try {
            if (factureInfo == null || !factureInfo.isHasDate()) {
                dateFinder = new DateFinder(learningEngine, rcs);
        ...
        // Period End Date.
        PeriodEndDateFinder periodEndDateFinder = null;
        try {
            if (factureInfo == null || !factureInfo.isHasPeriodEndDate()) {
                periodEndDateFinder = new PeriodEndDateFinder(learningEngine, rcs, dateFinder.GetValue());
                periodEndDateFinder.Find(documents);
                _factureInfo.setPeriodEndDateUsedKeyValues(periodEndDateFinder.GetAllKeyValues());
                // Dévalider les valeurs non valides.
                if (periodEndDateFinder.getValidValue() != null) {
                    periodEndDateFinder.getValidValue().setValue(periodEndDateFinder.GetValue());
                    _factureInfo.setPeriodEndDateKeyValue(periodEndDateFinder.getValidValue());
                }
            }
        } catch (Exception e) {
            log.error("Period End Date not found :" + e);
        }

        // Period Start Date.
        try {
            if (factureInfo == null || !factureInfo.isHasPeriodStartDate()) {
                PeriodStartDateFinder periodStartDateFinder = new PeriodStartDateFinder(learningEngine, rcs, periodEndDateFinder.GetValue());
                periodStartDateFinder.Find(documents);
                _factureInfo.setPeriodStartDateUsedKeyValues(periodStartDateFinder.GetAllKeyValues());
                // Dévalider les valeurs non valides.
                if (periodStartDateFinder.getValidValue() != null) {
                    periodStartDateFinder.getValidValue().setValue(periodStartDateFinder.GetValue());
                    _factureInfo.setPeriodStartDateKeyValue(periodStartDateFinder.getValidValue());
                }
            }
        } catch (Exception e) {
            log.error("Period Start Date not found :" + e);
        }
            ...
            try {
                ...
                if (!Facture.extractHtTvaTtc(htFinder, tvaFinder, ttcFinder, tvaSuppFinder, null, false, documents, _factureInfo)) {
                    ...
                    if (!Facture.extractHtTvaTtc(htFinder, tvaFinder, ttcFinder, tvaSuppFinder, null, true, documents, _factureInfo)) {
                        ...
                        BicNumberFinder bicFinder = new BicNumberFinder(learningEngine);
                        Facture.extractHtTvaTtc(htFinder, tvaFinder, ttcFinder, null, bicFinder, true, documents, _factureInfo);
                    }
                }
            } catch (Exception e) {
