...
@Log4j2
public class AccountNumberFinder extends KeyFinder<String> implements Serializable {
    ...
    public AccountNumberFinder() throws Exception {

        super(new Key(KeyType.ACCOUNT_NUMBER));
    }

    public AccountNumberFinder(LearningEngine learningEngine) throws Exception {

        super(new Key(KeyType.ACCOUNT_NUMBER), learningEngine, null);
    }
    /// <summary>
    ...
    private boolean _CheckValue(KeyValue keyValue, _Out<String> value) {
        String accountNumberStr = TextBlock.GetSequenceString(keyValue.getValueBlocks());

        for (int i = 0; i < accountNumberStr.length(); i++) {
            if (i + 11 < accountNumberStr.length())
                value.set(StringHelper.substring(accountNumberStr, i, 11));
            else
                value.set(StringHelper.substring(accountNumberStr, i, accountNumberStr.length() - i));

            if (NumberHelper.IsAccountNumber(value.get()))
                return true;
        }
    ...
    public LearningEntry GetLearningEntry(KeyValue keyValue, _Out<Boolean> isValidValue) {
        if (isValidValue.get()) {
            String accountNumber = GetValue(keyValue);
            return this.getLearningEngine().GetEntry(KeyType.ACCOUNT_NUMBER, accountNumber);
        } else
            return null;
    }
