...
import com.accounting_manager.bank_statement_engine.Classes.geometry.GeometryHelper;
...
//    }
    ...
    /// <summary>
    /// Vérifie si une valeur donnée est un numero bic ou non.
    /// Si c'est un bic alors un bloc virtuelle sera crée et le
    ...
    public boolean CheckBicValue(KeyValue keyValue, List<Block> allBlocks) throws Exception {
        ...
        if (NumberHelper.IsBicNumber(NumberHelper.CleanFloatSubstring(cleanStr))) {
    ...
    /// <summary>
    /// Vérifie si une valeur donnée est un numero RIB ou non.
    /// Si c'est un bic alors un bloc virtuelle sera crée et le
    /// poids peut être modifié.
    /// </summary>
    /// <param name="keyValue">Instance d'une clé/valeur.</param>
    /// <param name="allBlocks">Liste de tous les blocs.</param>
    public boolean CheckRibValue(KeyValue keyValue, List<Block> allBlocks) throws Exception {
        List<TextBlock> cleanBlocks = NumberHelper.CleanFloatBlocks(keyValue.getValueBlocks(), keyValue.getValueBaseBlock() != null ? keyValue.getValueBaseBlock() : keyValue.getValueBlocks().get(0));
        String cleanStr = TextBlock.GetSequenceString(cleanBlocks);

        if (NumberHelper.IsRibNumber(NumberHelper.CleanFloatSubstring(cleanStr))) {
            // Créer un bloc virtuel + changement du poids.
            if (cleanStr.length() != TextBlock.GetSequenceString(keyValue.getValueBlocks()).length()) {
                TextBlock virtualValueBlock = new TextBlock(cleanBlocks);
                // Re-calculer les données du poids.
    ...
    /// <summary>
    /// Vérifie si une valeur donnée est un numero IBAN ou non.
    /// Si c'est un bic alors un bloc virtuelle sera crée et le
    /// poids peut être modifié.
    /// </summary>
    /// <param name="keyValue">Instance d'une clé/valeur.</param>
    /// <param name="allBlocks">Liste de tous les blocs.</param>
    public boolean CheckIbanValue(KeyValue keyValue, List<Block> allBlocks) throws Exception {
        List<TextBlock> cleanBlocks = NumberHelper.CleanFloatBlocks(keyValue.getValueBlocks(), keyValue.getValueBaseBlock() != null ? keyValue.getValueBaseBlock() : keyValue.getValueBlocks().get(0));
        String cleanStr = TextBlock.GetSequenceString(cleanBlocks);

        if (NumberHelper.IsIbanNumber(NumberHelper.CleanFloatSubstring(cleanStr))) {
            // Créer un bloc virtuel + changement du poids.
            if (cleanStr.length() != TextBlock.GetSequenceString(keyValue.getValueBlocks()).length()) {
                TextBlock virtualValueBlock = new TextBlock(cleanBlocks);
                // Re-calculer les données du poids.
