...
@Log4j2
public class RibNumberFinder extends KeyFinder<String> implements Serializable {
    ...
    public RibNumberFinder() throws Exception {

        super(new Key(KeyType.RIB_NUMBER));
    }

    public RibNumberFinder(LearningEngine learningEngine) throws Exception {

        super(new Key(KeyType.RIB_NUMBER), learningEngine, null);
    }
    /// <summary>
    ...
    private boolean _CheckValue(KeyValue keyValue, _Out<String> value) {
        String ribNumberStr = TextBlock.GetSequenceString(keyValue.getValueBlocks());

        for (int i = 0; i < ribNumberStr.length(); i++) {
            if (i + 27 < ribNumberStr.length())
                value.set(StringHelper.substring(ribNumberStr, i, 27));
            else
                value.set(StringHelper.substring(ribNumberStr, i, ribNumberStr.length() - i));

            if (NumberHelper.IsRibNumber(value.get()))
                return true;
        }
    ...
    public LearningEntry GetLearningEntry(KeyValue keyValue, _Out<Boolean> isValidValue) {
        if (isValidValue.get()) {
            String ribNumber = GetValue(keyValue);
            return this.getLearningEngine().GetEntry(KeyType.RIB_NUMBER, ribNumber);
        } else
            return null;
    }
