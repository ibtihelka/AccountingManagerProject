...
/**
 * Controller for managing documents within folders.
 */
@RestController
...
public class DocumentController {
    ...
    public ResponseEntity<Page<Document>> getAllDocuments(
        ...
        Page<Document> documents = documentService.getDocuments(token, folderId, query, page, size);
    ...
    public ResponseEntity<Page<Document>> getAllDocumentsForAdmin(
            @RequestParam(defaultValue = "") Long folder,
            @RequestParam(defaultValue = "") String query,
            @RequestParam(defaultValue = "") String type,
            @RequestParam(name = "page", required = false, defaultValue = "0") int page,
            @RequestParam(name = "size", required = false, defaultValue = "8") int size) {
        Page<Document> documents = documentService.getDocumentsForAdmin(folder, query, type, page, size);
        return ResponseEntity.ok(documents);
    }
    ...
    public ResponseEntity<Map<String, Object>> createDocument(
        ...
        if (folderOptional.isPresent()) {
            Map<String, Object> response = documentService.createDocuments(folderId, files, folderOptional.get().getType().getCodeType());
    ...
    public ResponseEntity<Document> updateInvoice(@PathVariable Long id, @RequestBody Invoice updatedDocument) {
        Optional<Document> updated = documentService.updateInvoice(id, updatedDocument);
    ...
    public ResponseEntity<Document> updateBankStatement(@PathVariable Long id, @RequestBody BankStatement updatedDocument) {
        Optional<Document> updated = documentService.updateBankStatement(id, updatedDocument);
        return updated.map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
    }
    ...
    public ResponseEntity<Document> updateInvoices(@RequestBody Set<Invoice> updatedDocuments) {
    ...
    public ResponseEntity<Document> updateBankStatements(@RequestBody Set<BankStatement> updatedDocuments) {
    ...
    public ResponseEntity<Void> deleteDocumentPermanently(@PathVariable Long id) {
