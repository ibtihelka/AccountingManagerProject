...
/**
 * Controller for managing third parties related to management operations using NoSQL.
 */
@RestController
@RequestMapping("/api/v1/management/thirdparty")
@CrossOrigin(origins = "*")
@Profile("nosql")
public class ThirdPartyControllerNoSql {

    private final ThirdPartyServiceNoSql thirdPartyServiceNoSql;

    public ThirdPartyControllerNoSql(ThirdPartyServiceNoSql thirdPartyServiceNoSql) {
    ...
    @GetMapping
    public ResponseEntity<Page<ThirdPartyNoSql>> getAllThirdParties(@RequestParam(required = false, defaultValue = "") String query,
        ...
        Page<ThirdPartyNoSql> thirdParties = thirdPartyServiceNoSql.getAllThirdParties(query, discriminator.toLowerCase(), page, size);
    ...
    @GetMapping("/{id}")
    public ResponseEntity<ThirdPartyNoSql> getThirdPartyById(@PathVariable String id) {
        Optional<ThirdPartyNoSql> thirdPartyOptional = thirdPartyServiceNoSql.getThirdPartyById(id);
    ...
    public ResponseEntity<Page<ThirdPartyNoSql>> getAccountantClients(@PathVariable String id,
                                                                      @RequestParam(name = "query", required = false, defaultValue = "") String query,
                                                                      @RequestParam(name = "page", required = false, defaultValue = "0") int page,
                                                                      @RequestParam(name = "size", required = false, defaultValue = "8") int size) {
        try {
            Page<ThirdPartyNoSql> thirdParties = thirdPartyServiceNoSql.getAccountantClients(id, query, page, size);
            return ResponseEntity.ok(thirdParties);
    ...
    public ResponseEntity<Page<ThirdPartyNoSql>> getClientAccountants(@PathVariable String id,
                                                                      @RequestParam(name = "query", required = false, defaultValue = "") String query,
                                                                      @RequestParam(name = "page", required = false, defaultValue = "0") int page,
                                                                      @RequestParam(name = "size", required = false, defaultValue = "8") int size) {
        try {
            Page<ThirdPartyNoSql> thirdParties = thirdPartyServiceNoSql.getClientAccountants(id, query, page, size);
            return ResponseEntity.ok(thirdParties);
        } catch (IllegalArgumentException e) {
            return ResponseEntity.notFound().build();
        }
    ...
    public ResponseEntity<?> createClientForAccountant(
        ...
        try {
            ThirdPartyNoSql response = thirdPartyServiceNoSql.createThirdParty(id, request);
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteThirdPartyAsAdmin(@PathVariable String id) {
        thirdPartyServiceNoSql.deleteThirdPartyAsAdmin(id);
    ...
    @DeleteMapping()
    public ResponseEntity<Void> deleteThirdPartyAsAAccountant(@RequestParam String accountantId, @RequestParam String clientId) {
        thirdPartyServiceNoSql.deleteThirdPartyAsAccountant(accountantId, clientId);
        return ResponseEntity.ok().build();
    }
