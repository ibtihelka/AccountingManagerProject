...
import finders.AmountFinder;
import finders.DateFinder;
import finders.DiscountFinder;
import finders.HtFinder;
import finders.NumberFactureFinder;
import finders.NumberPageFinder;
import finders.NumberRcsFinder;
import finders.NumberSiretFinder;
import finders.NumberTvaFinder;
import finders.TtcFinder;
import finders.TvaFinder;
import finders.TvaSuppFinder;
import finders.TypeFactureFinder;
...
import All.ThreadException;
import All._Out;
import cpp.CppOcrEngine;
...
public class Facture implements Serializable {
	...
	private static final long serialVersionUID = 7980598540359269499L;
			...
			public static String getUniqueFileName (String filePath, String separator)
			{
				...
				if (indexOf > -1)
				{
					ext = filePath.substring (indexOf + 1, filePath.length() - 1 - indexOf);
			...
			/// <returns>Chaine de caractères qui représente le nom du flux.</returns>
			private static String getDocStreamName (DocumentProcessing processingType, DocumentRotation rotationType){
				
				return processingType + "-" + rotationType;
			}
			...
			public String GetDrawFileName (DocumentProcessing processingType, DocumentRotation rotationType){
				
				return this.fullFileName + "-" + Facture.getDocStreamName (processingType, rotationType).toLowerCase () + ".draw.jpg";
			}
			...
			public Document CreateDocument (
									...
									DocumentProcessing processingType, OcrEngine ocrEngine, DocumentRotation rotationType, String outputDir) throws IOException
			{
				...
				{
					// Lancer le processus.
					String		processArgs = Facture.FormatProcessArgs (new String [] {developerSN, engineLibPath, fileName, String.valueOf(rotationType)});
			...
			public FactureInfo startProcessing (ProcessingOption options) throws Exception
			{
				...
				// Traitement 1.
				Document []		allDocuments = null;
				FactureInfo		factureInfo = _StartProcessing (null, DocumentProcessing.NONE, options, new Document [0],  allDocuments);
				// Traitement 2.
				if (factureInfo.isRejected())
					factureInfo = _StartProcessing (factureInfo, DocumentProcessing.FILTER_A, options, allDocuments,  allDocuments);
			...
			private FactureInfo _StartProcessing (
										...
										Document [] includeDocuments,
										Document [] allDocuments) throws Exception
			{
				// Premier traitement : sans rotations.
				FactureInfo		_factureInfo = _StartProcessing (factureInfo, processingType, options, DocumentRotation.NONE, includeDocuments,  allDocuments);
				...
				if (_factureInfo.isType1StronglyRejected() || _factureInfo.isType2StronglyRejected())
				{
					...
					_factureInfo = _StartProcessing (_factureInfo, processingType, options, DocumentRotation.ROTATION_1, includeDocuments,  allDocuments);
					...
					if (_factureInfo.isType1StronglyRejected() || _factureInfo.isType2StronglyRejected())
					{
						...
						_factureInfo = _StartProcessing (_factureInfo, processingType, options, DocumentRotation.ROTATION_2, includeDocuments,  allDocuments);
					}
				}
			...
			private FactureInfo _StartProcessing (
										FactureInfo factureInfo,
										DocumentProcessing processingType, ProcessingOption options, DocumentRotation rotationType,
										Document [] includeDocuments,
										 Document [] allDocuments) throws Exception
			{
				// Fixer le nom du flux de données.
				String				streamName = Facture.getDocStreamName (processingType, rotationType);
				// Extraire le document à partir de la cache.
				FactureCache		documentCache = this.multiFacture.getFacturesCache().get(this.index);
				Document			document = Document.Deserialize (documentCache.GetDocStream (streamName));
				boolean				mustDraw = document == null;

				if (document == null)
				{
					document = CreateDocument (
									this.multiFacture.getDeveloperSN(), this.multiFacture.getEngineLibPath(), this.multiFacture.isCreateProcess(), this.multiFacture.getProcessFileName(),
									processingType, this.multiFacture.getOcrEngine(), rotationType, this.multiFacture.getOutputDir());
				...
				{
					String		drawFileName = GetDrawFileName (processingType, rotationType);
				...
				allDocuments = new Document [includeDocuments.length + 1];
				
				allDocuments [allDocuments.length - 1] = document;
				allDocuments=Arrays.copyOfRange(includeDocuments, 0, includeDocuments.length);
				FactureInfo		_factureInfo = _StartProcessing (allDocuments, factureInfo);
			...
			/// <returns>Instance de type <see cref="FactureInfo" /> qui représente les informations de la facture.</returns>
			private FactureInfo _StartProcessing (Document [] documents, FactureInfo factureInfo) throws Exception
			{
			...
			private static boolean extractHtTvaTtc (
										...
										Document [] documents, FactureInfo factureInfo)
			{
				...
				KeyValue []		htKeyValues = htFinder.GetAllKeyValues ();
				KeyValue []		tvaKeyValues = tvaFinder.GetAllKeyValues ();
				KeyValue []		ttcKeyValues = ttcFinder.GetAllKeyValues ();
				KeyValue []		tvaSuppKeyValues = amountFinder == null ? tvaSuppFinder.GetAllKeyValues () : null;
				KeyValue []		amountKeyValues = amountFinder != null ? amountFinder.GetAllKeyValues () : null;
			    ...
			    _Out<KeyValue>			validHtKeyValue = new  _Out<KeyValue>();
			    _Out<KeyValue>			validTvaKeyValue = new  _Out<KeyValue>();
			    _Out<KeyValue>			validTtcValue = new  _Out<KeyValue>();
			    _Out<KeyValue>			validTvaSuppValue = new  _Out<KeyValue>();
			...
			private static KeyValue [] mergeKeyValues (KeyValue [] keyValues1, KeyValue [] keyValues2)
			{
				KeyValue []		keyValues = new KeyValue [keyValues1.length + keyValues2.length];

				
				keyValues=Arrays.copyOfRange(keyValues1, 0, keyValues1.length);
				
				keyValues=Arrays.copyOfRange(keyValues2, keyValues1.length, keyValues2.length);
				return keyValues;
			}
			...
			private static void fixHtTvaTtcValues (
										...
										KeyValue [] htKeyValues, KeyValue [] tvaKeyValues, KeyValue [] ttcKeyValues, KeyValue [] tvaSuppKeyValues)
			{
				// HT.
										 ...
										 _Out<KeyValue> validHtKeyValue,  _Out<KeyValue> validTvaKeyValue,  _Out<KeyValue> validTtcValue,  _Out<KeyValue> validTvaSuppValue)
			{
				...
				validTva.set(0.0);
				validTtc.set(0.0) ;
				validHtKeyValue = new _Out<KeyValue>();
				validTvaKeyValue = new _Out<KeyValue>();
				validTvaSuppValue = new _Out<KeyValue>();
			...
			private static boolean extractHtTvaTtcWithTVARates (
										KeyValue [] htKeyValues, KeyValue [] tvaKeyValues, KeyValue [] ttcKeyValues,
										 _Out<Double> validHt,  _Out<Double> validTva,  _Out<Double> validTtc,
										 _Out<KeyValue> validHtKeyValue,  _Out<KeyValue> validTvaKeyValue,  _Out<KeyValue> validTtcValue)
			{
				...
				validHtKeyValue= new _Out<KeyValue>();
				validTvaKeyValue = new _Out<KeyValue>();
				validTtcValue = new _Out<KeyValue>();
			...
			{
				KeyValue []		keyValues = keyFinder.GetAllKeyValues ();
			...
			public static void DrawKeyValues (FactureInfo factureInfo, String imagePath, ImageType imageType) throws Exception
			{
				...
				KeyValue []				allKeyValues = factureInfo.GetAllUsedKeyValues ();
				...
				for(Entry<KeyType,List<KeyValue>> keyValuePair : keyValuesByType.entrySet())
				{
			...
			private static void _CreateBox (
									boolean isDebugBox, Document document,Region region, String keyTypeStr, int index,
			...
			private static void adjustRegion (
									Region region, Document document,
