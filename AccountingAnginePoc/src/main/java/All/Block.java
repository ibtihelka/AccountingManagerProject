	...
	public void setDocument(Document document){
		{
			...
			for (int i = 0; i < this.innerBlocks.length; i ++)
			{this.innerBlocks [i].document = document;}
		}
	}
			...
			public Block (Region region, Block [] innerBlocks)
			{
			...
			public static Region GetRegion (Block [] innerBlocks)
			{
				int				left = 0;
				int				top = 0;
				int				right = 0;
			...
			public Block [] GetInnerBlocks ()
			{return (Block [])this.innerBlocks.clone();}
			
			/// <summary>
			/// Ajoute tous les blocs enfants dans la liste pass e en paramètre.
			/// </summary>
			...
			public Block [] GetAllBlocks (DocumentZone documentZone, boolean addCurrentBlock)
			{
				...
				return (Block[]) allInnerBlocks.toArray ();
			}
			...
			/// <param name="documentZone">Zone du filtrage. <see cref="FacDocumentZone.UNDEFINED" /> indique qu'aucun filtre de zone ne doit être appliqué.</param>
			/// <returns>Tableau de type <see cref="FacBlock" />.</returns>
			public Block [] getAllBlocks (DocumentZone documentZone)
			...
			public static <T> Block [] FilterBlocks  (Block [] blocks, Class<T> type)
			{
				return (Block[]) new ArrayList<Block> ().stream().filter(a -> type.isInstance(a)).collect(Collectors.toList()).toArray();
			}
}