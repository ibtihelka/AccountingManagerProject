...
public abstract class KeyFinder<T> {
	/// <summary>
			...
			public void Find (Document [] documents)
			{
				// Récupérer tous les blocs à parcourir.
				List<Block>			allBlocksList = new ArrayList<Block> ();

				for(Document document : documents)
				{Collections.addAll (allBlocksList,document.GetAllBlocks ());}

				Block []				allBlocks = (Block[]) allBlocksList.toArray ();
				// Extraire les clés valides.
				TextBlock [] []		keyBlocks;
				TextBlock []			_keyBlocks;
				TextBlock			keyBlock = null;

				List<KeyValue>		_keyValues = new ArrayList<KeyValue> ();
				TextBlock []			unmarkedSeq;

				KeyString []			keyStrings = null;
				List<Block>			newBlocks = new ArrayList<Block> ();
				TextBlock []			_newBlocks = null;

				for (int i = 0; i < allBlocks.length; i ++)
				{
					if(allBlocks [i] instanceof TextBlock)
					keyBlock = (TextBlock) allBlocks [i];
					...
					if (keyBlock != null && !keyBlock.is_isSeqMarked() && !keyBlock.is_isKeyMarked())
					{
						...
						if (unmarkedSeq.length > 0 && (
							keyBlocks = FindKey (unmarkedSeq, allBlocks, keyStrings, _newBlocks)) != null)
						{
							// Créer l'instance clé/valeur.
							for (int j = 0; j < keyBlocks.length; j ++)
							{
								_keyBlocks = keyBlocks [j];
								_keyValues.add (new KeyValue (this.key, _keyBlocks, keyStrings [j]));
								// Marquer les blocs.
								for(TextBlock keyTextBlock : _keyBlocks)
								{keyTextBlock.set_isSeqMarked(true);}
							}

							Collections.addAll (newBlocks,_newBlocks); 
						}
						// Marquer la séquence.
						TextBlock.markSequence (unmarkedSeq, true);
					}
				}
				// Démarquer les blocs.
				...
				if (newBlocks.size() > 0)
				{
					Block []		_allBlocks = new Block [allBlocks.length + newBlocks.size()];
					
					_allBlocks=Arrays.copyOfRange(allBlocks, 0, allBlocks.length);
					_allBlocks=Arrays.copyOfRange((Block[])newBlocks.toArray(), allBlocks.length, newBlocks.size());
					
					allBlocks = _allBlocks;
				}

				KeyValue []		keyValues = (KeyValue[]) _keyValues.toArray ();
				...
				keyValues = (KeyValue[]) _keyValues.toArray ();
				...
				for(KeyValue keyValue : keyValues)
				{
					...
					// Vérifier la valeur.
					if (keyValue.getValueBlocks() != null && !isValueChecked)
						checkResult = CheckValue (keyValue, allBlocks);

					keyValue.setValidValue( checkResult);
				}
				// Extraire la liste totale des clés/valeurs.
				this.keyValues = keyValues;
				KeyValue []		allKeyValues = GetAllKeyValues ();
				// Vérifier globalement les valeurs.
				CheckAllValues (allKeyValues, allBlocks);
			...
			public TextBlock [] [] FindKey (
												TextBlock [] seqBlocks, Block [] allBlocks,
												KeyString [] keyStrings, TextBlock [] newBlocks)
			{
				...
				newBlocks = (TextBlock[]) _newBlocks.toArray ();
				keyStrings = (KeyString[]) _keyStrings.toArray ();

				return _keyBlocks.size() > 0 ?(TextBlock[][]) _keyBlocks.toArray () : null;
			}
			...
			private void _FindKey (
							TextBlock [] seqBlocks, Block [] allBlocks,
							List<TextBlock []> keyBlocks, List<KeyString> keyStrings, List<TextBlock> newBlocks)
			{
				// Déclarer les variables.
				String					seqBlocksStr = TextBlock.GetSequenceString (seqBlocks);

				TextBlock []			newLeftSeqBlocks = null, newRightSeqBlocks = null;

				TextBlock []			_keyBlocks = null;
				TextBlock []			_newBlocks = null;
				...
				for(KeyString keyString : this.key.getAllKeyStrings())
				{
					...
					{
						keyBlocks.add (_keyBlocks);
						keyStrings.add (keyString);
						Collections.addAll (newBlocks,_newBlocks);
						// Gauche.
						if (newLeftSeqBlocks.length > 0)
							_FindKey (newLeftSeqBlocks, allBlocks, keyBlocks, keyStrings, newBlocks);
						// Droite.
						if (newRightSeqBlocks.length > 0)
							_FindKey (newRightSeqBlocks, allBlocks, keyBlocks, keyStrings, newBlocks);
			...
			private boolean GetKeyBlocks (
								KeyString keyString, TextBlock [] seqBlocks, String seqBlocksStr,
								TextBlock [] keyBlocks, TextBlock [] newLeftSeqBlocks, TextBlock [] newRightSeqBlocks, TextBlock [] newBlocks)
			{
				...
				if (keyStartIndex > -1)
				{
					...
					_Out<TextBlock> right = new _Out<TextBlock>() ;
					_Out<TextBlock> left =new _Out<TextBlock>();
					TextBlock center;
					TextBlock			keyBlock;
					...
					keyBlocks = new TextBlock [keyString.getStringCharsCount()];
					...
					for (int i = 0; i < _keyBlocks.size(); i ++)
					{
						...
						// Créer les blocs virtuels de la clé.
						if (makeKeyBlocks || i == _keyBlocks.size() - 1)
						{
							...
							// OK.
							virtuelKeyBlock = new TextBlock ((TextBlock[]) __keyBlocks.toArray ());
							keyBlocks [keyCharIndex] = virtuelKeyBlock;
							// Vérifier l'espacement.
							if (keyCharIndex > 0)
							{
								width = virtuelKeyBlock.getRegion().getLeft() - keyBlocks [keyCharIndex - 1].getRegion().getRight();
								...
								if (keyChar.getMinSeperatorWidth() >= 0 && width < minWidth ||
									keyChar.getMaxSeperatorWidth() >= 0 && width > maxWidth)
								{
									returnFalse = true;
									break;
								}
							...
							// Incrémenter la clé.
							keyCharIndex ++;
							if (keyCharIndex < keyBlocks.length)
							{
					...
					if (!returnFalse)
					{
						newLeftSeqBlocks = (TextBlock[]) _newLeftSeqBlocks.toArray ();
						newRightSeqBlocks = (TextBlock[]) _newRightSeqBlocks.toArray ();
						newBlocks = (TextBlock[]) _newBlocks.toArray ();
				...
				keyBlocks = null;
				newLeftSeqBlocks = null;
				newRightSeqBlocks = null;
				newBlocks = null;
			...
			public void MarkKeyBlocks (TextBlock [] keyBlocks, KeyString keyString)
			{
				for (int i = 0; i < keyBlocks.length; i ++)
				{
					if (!keyString.GetStringChar (i).isShared())
						keyBlocks [i].set_isKeyMarked( true);
				}
			}
			/// <summary>
			...
			public abstract TextBlock [] FindValue (KeyValue keyBlock, Block [] allBlocks,  boolean isValueChecked,  boolean checkResult);
			/// <summary>
			...
			public abstract boolean CheckValue (KeyValue keyValue, Block [] allBlocks);
			/// <summary>
			...
			public int GetZoneWeight (DocumentZone zoneRef, DocumentZone zone, Document document)
			{
				Region		zoneRefRegion = document.GetZoneRegions (zoneRef) [0];
				Region []	zoneRegions = document.GetZoneRegions (zone);
			...
			{
				double			zoneWeight = 0;
				Document		document = keyValue.getVirtualKeyBlock().getDocument();
			...
			public void CheckLearnedValues (KeyValue [] keyValues)
			{
			...
			public  void CheckAllValues (KeyValue [] keyValues, Block [] allBlocks)
			{
				...
				for(KeyValue keyValue : keyValues)
				{
					if (keyValue.isValidValue())
					{
						...
						{
							weight = GetWeight(keyValue);
							
							hWeights.computeIfAbsent(weight, k -> new ArrayList<KeyValue> ()).add(keyValue);
			...
			//	Array.Sort<Float,List<KeyValue>> (weights, __keyValues);
				...
				if (nullKeyValue != null)
				{
					...
					System.arraycopy(weights, 0, tmpWeights, 0, weights.length + 1);
					
					tmpWeights [tmpWeights.length - 1] = new Float(-1);

					List<KeyValue> []	tmpKeyValues = (List<KeyValue>[]) new List [__keyValues.length + 1];
					//__keyValues.CopyTo (tmpKeyValues, 0);
					
					System.arraycopy(__keyValues, 0, tmpKeyValues, 0, __keyValues.length + 1);
					
					
					
					//tmpKeyValues [tmpKeyValues.length - 1] = new ArrayList<KeyValue> (new KeyValue [] {nullKeyValue});
					...
					if (!invalidValues)
					{
						if (_keyValues != null)
						{
							if (_keyValues.size() > 1)
								CheckSameWeightValues ((KeyValue [])_keyValues.toArray (), weight);
			...
			public void CheckSameWeightValues (KeyValue [] keyValues, float weight)
			{
				Map<Object, List<KeyValue>> votes = new HashMap<>();
				List<KeyValue>						votesLists = null;
				Object									value;
			...
			public static void InvalidKeyValuesExcept (KeyValue [] keyValues, KeyValue keyValueExcept)
			{
				for(KeyValue keyValue : keyValues)
				{
			...
			public KeyValue GetKeyValue (KeyValue [] keyValues)
			{
				for(KeyValue keyValue :  keyValues)
				{
					if (keyValue.isValidValue())
			...
			public boolean SetRightTextAsValue (KeyValue keyValue, Block valueStartBlock, float weight, Block [] allBlocks)
			{
			...
			private Block [] SelectBlocks (
										KeyValue keyValue, Block [] blocks,
										_Out<Integer>[] alignements, _Out<Integer>[] distances, _Out<Integer>[] shadows,
										Block [] allBlocks,
										boolean checkValue,
										Float[] selWeights, TextBlock [] [] selValueBlocks)
			{
				List<Block>			selBlocks = new ArrayList<Block> ();
				List<Float>				_selWeights = new ArrayList<Float> ();
				List<TextBlock []>	_selValueBlocks = new ArrayList<TextBlock []> ();
				...
				{
					block = blocks [i];
					weight = GetBlockWeight (block, alignements [i].get(), distances [i].get(), shadows [i].get());
						...
						SetRightTextAsValue (keyValue, block, weight, allBlocks)))
					{
						...
						_selWeights.add (GetLearningBlockWeight (GetLearningEntry (keyValue, checkValue ? true : keyValue.isValidValue()), checkValue ? keyValue.getVirtualValueBlock() : block, checkValue ? keyValue.getWeight() : weight));
						_selValueBlocks.add (checkValue ? keyValue.getValueBlocks() : new TextBlock [] {(TextBlock)block});
					}
				}

				selWeights = (Float[]) _selWeights.toArray ();
				selValueBlocks = (TextBlock[][]) _selValueBlocks.toArray ();
			...
			private boolean Select (
							...
							Block [] allBlocks,
							boolean checkValue,
							Float weight,  TextBlock [] valueBlocks)
			{
				Block []			selBlocks;
				Float []			selWeights = null;
				TextBlock [] []	selValueBlocks = null;
			...
			static void ArryasSort( Float[] selWeights, TextBlock[][] selValueBlocks) {
				Float tempselWeights;
				TextBlock [] tempselValueBlocks;
		        
		        
		        for (int i=0; i<selValueBlocks.length-1; i++) {
		            for (int j=0; j<selWeights.length-i-1; j++) {
		                if (selWeights[j] < selWeights[j+1]) {
		                    tempselWeights = selWeights[j];
		                    tempselValueBlocks = selValueBlocks[j];

		                    selWeights[j] = selWeights[j+1];
		                    selValueBlocks[j] = selValueBlocks[j+1];

		                    selWeights[j+1] = tempselWeights;
		                    selValueBlocks[j+1] = tempselValueBlocks;
		                }
		            }
		        }
		    }
			...
			public KeyValue [] FindValuesNullKey (KeyValue nullKeyValue, Block [] allBlocks)
			{
			    ...
			    for(Block block : allBlocks)
			    {
					...
					if (textBlock != null && !textBlock.is_isSeqMarked())
			        {
						...
						// Marquer la séquence.
						TextBlock.markSequence (seqBlocks, true);
			        }
			    }
				// Marquer les blocs.
				TextBlock.markSequence (allBlocks, false);
				...
				return (KeyValue[]) keyValues.toArray ();
			}
			/// <summary>
			...
			public boolean FindValueInTextBlocUsingRight (
								KeyValue keyValue, Block [] allBlocks, boolean checkValue, 
								 float weight,  TextBlock [] valueBlocks)
			{
				TextBlock		virtualKeyBlock = keyValue.getVirtualKeyBlock();
				// Faire la recherche.
				Block []			blocks;
				_Out<Integer> []				alignements = null;
				_Out<Integer> []				distances = null;
				_Out<Integer> []				shadows = null;
			...
			public TextBlock [] FindValueInTextBlock (KeyValue keyValue, Block [] allBlocks, boolean isValueChecked,  boolean checkResult)
			{
				// Faire la recherche.
				TextBlock []		valueBlocks = null, selValueBlocks;
				float				weight = 0, selWeight, _weight, _selWeight;
				...
				_weight = WEIGHT_RIGHT_DIR_COEFF * weight;
				...
				_weight = WEIGHT_DOWN_DIR_COEFF * weight;
				...
				_weight =WEIGHT_UP_DIR_COEFF * weight;
			...
			public KeyValue [] FindSuppValues (KeyValue keyValue, boolean horizontalSeq, Block [] allBlocks)
			{
				List<KeyValue>		suppKeyValues = new ArrayList<KeyValue> ();
				KeyValue				suppKeyValue;

				float					weight = 0;
				TextBlock []			valueBlocks = null;

				while (true)
				{
					suppKeyValue = keyValue.getSuppKeyValue (keyValue.getValueBlocks());
							...
							FindValueInTextBlocUsingDown (suppKeyValue, allBlocks, true,  weight, valueBlocks)))
					{
						...
						if (weight < (horizontalSeq ? HSUPP_MAX_WEIGHT : VSUPP_MAX_WEIGHT))
						{
							suppKeyValue.setWeight(weight);
							suppKeyValue.setValueBlocks(valueBlocks);
				...
				return (KeyValue[]) suppKeyValues.toArray ();
			}
			...
			public KeyValue [] GetAllKeyValues ()
			{
				...
				for(KeyValue keyValue : this.keyValues)
				{
					if (keyValue.getSuppKeyValues() != null)
						Collections.addAll (allKeyValues,keyValue.getSuppKeyValues());
				}

				return (KeyValue[]) allKeyValues.toArray ();
			}
			...
			/// <param name="allBlocks">Liste de tous les blocs objects de la recherche.</param>
			public void CheckSuppAllValues (KeyValue [] keyValues, boolean isSecondProcessing, Block [] allBlocks)
			{
				// Valeurs supplémentaires.
				KeyValue []		hSuppKeyValues, vSuppKeyValues;
				KeyValue []		suppKeyValues;

				for(KeyValue keyValue : keyValues)
				{
					if (keyValue.isValidValue())
					{
						hSuppKeyValues = isSecondProcessing ? FindSuppValues (keyValue, true, allBlocks) : null;
						vSuppKeyValues = FindSuppValues (keyValue, false, allBlocks);

						if (hSuppKeyValues != null)
						{
							suppKeyValues = new KeyValue [vSuppKeyValues.length + hSuppKeyValues.length];
							
							suppKeyValues=Arrays.copyOfRange(vSuppKeyValues, 0, vSuppKeyValues.length);
							suppKeyValues=Arrays.copyOfRange(hSuppKeyValues, vSuppKeyValues.length, hSuppKeyValues.length);
							
						}
						else
							suppKeyValues = vSuppKeyValues;
			...
			public boolean CheckAmountValue (KeyValue keyValue, Block [] allBlocks) throws Exception
			{
				TextBlock []		cleanBlocks = NumberHelper.CleanFloatBlocks (keyValue.getValueBlocks(), keyValue.getValueBaseBlock() != null ? keyValue.getValueBaseBlock() : keyValue.getValueBlocks() [0]);
